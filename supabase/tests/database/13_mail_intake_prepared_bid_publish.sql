...
insert into auth.users (id, email, raw_user_meta_data, raw_app_meta_data) values
  ...
  ('51000000-0000-4000-8000-000000000004', 'selected-seller@test.local', '{}', '{}');
update app_private.user_accounts set status = 'active' where user_id in ('51000000-0000-4000-8000-000000000001', '51000000-0000-4000-8000-000000000002', '51000000-0000-4000-8000-000000000004');
...
insert into app_private.organization_memberships (id, user_id, organization_id, role, status) values
  ...
  ('53000000-0000-4000-8000-000000000004', '51000000-0000-4000-8000-000000000004', '52000000-0000-4000-8000-000000000004', 'trader', 'active');
...
select set_config('prepared_mail.zero_seller_id', public.ingest_mail_intake_item('test', 'prepared-mailbox', 'zero-seller-1', clock_timestamp(), 'Zero seller source')::text, true);
select set_config('prepared_mail.null_seller_id', public.ingest_mail_intake_item('test', 'prepared-mailbox', 'null-seller-1', clock_timestamp(), 'Null seller source')::text, true);
select set_config('prepared_mail.duplicate_seller_id', public.ingest_mail_intake_item('test', 'prepared-mailbox', 'duplicate-seller-1', clock_timestamp(), 'Duplicate seller source')::text, true);
reset role;
...
select is((select count(*) from public.list_trader_bids('53000000-0000-4000-8000-000000000004') where id = (select id from published_bid)), 1::bigint, 'selected SELLER can view the published BID');
set local role authenticated;
select set_config('request.jwt.claim.sub', '51000000-0000-4000-8000-000000000001', true);
...
select throws_ok($$select public.publish_mail_intake_bid('53000000-0000-4000-8000-000000000002', current_setting('prepared_mail.nonbuyer_id')::uuid, 1, 'V', 'P', 'W', clock_timestamp() + interval '1 day', null, array['vlsfo'], array[1]::numeric[], array[]::uuid[])$$, '42501', 'An active BUYER membership is required', 'TRADER cannot publish mail intake'); -- 25

reset role;
set local role authenticated;
select set_config('request.jwt.claim.sub', '51000000-0000-4000-8000-000000000003', true);
select throws_ok($$select public.publish_mail_intake_bid('53000000-0000-4000-8000-000000000003', current_setting('prepared_mail.inactive_id')::uuid, 1, 'V', 'P', 'W', clock_timestamp() + interval '1 day', null, array['vlsfo'], array[1]::numeric[], array[]::uuid[])$$, '42501', 'An active BUYER membership is required', 'inactive account cannot publish mail intake'); -- 26
...
select throws_ok($$select public.publish_mail_intake_bid('53000000-0000-4000-8000-000000000001', current_setting('prepared_mail.invalid_fuel_id')::uuid, 1, 'V', 'P', 'W', clock_timestamp() + interval '1 day', null, array[]::text[], array[]::numeric[], array['52000000-0000-4000-8000-000000000004'::uuid])$$, '22023', 'Fuel grades and quantities must be non-empty equal-length arrays', 'publish without valid fuel items is denied'); -- 28
...
select throws_ok($$select public.publish_mail_intake_bid('53000000-0000-4000-8000-000000000001', current_setting('prepared_mail.zero_seller_id')::uuid, 1, 'V', 'P', 'W', clock_timestamp() + interval '1 day', null, array['vlsfo'], array[1]::numeric[], array[]::uuid[])$$, '22023', 'At least one selected active SELLER is required', 'zero selected SELLER organizations are denied');
select throws_ok($$select public.publish_mail_intake_bid('53000000-0000-4000-8000-000000000001', current_setting('prepared_mail.null_seller_id')::uuid, 1, 'V', 'P', 'W', clock_timestamp() + interval '1 day', null, array['vlsfo'], array[1]::numeric[], null)$$, '22023', 'Selected SELLER organizations are required', 'null selected SELLER organizations are denied');
select throws_ok($$select public.publish_mail_intake_bid('53000000-0000-4000-8000-000000000001', current_setting('prepared_mail.duplicate_seller_id')::uuid, 1, 'V', 'P', 'W', clock_timestamp() + interval '1 day', null, array['vlsfo'], array[1]::numeric[], array['52000000-0000-4000-8000-000000000004'::uuid, '52000000-0000-4000-8000-000000000004'::uuid])$$, '22023', 'Selected SELLER organizations are duplicated', 'duplicate selected SELLER organizations are denied');
reset role;
select is((select count(*) from app_private.mail_intake_items where id in (current_setting('prepared_mail.missing_deadline_id')::uuid, current_setting('prepared_mail.invalid_fuel_id')::uuid, current_setting('prepared_mail.inactive_seller_id')::uuid, current_setting('prepared_mail.zero_seller_id')::uuid, current_setting('prepared_mail.null_seller_id')::uuid, current_setting('prepared_mail.duplicate_seller_id')::uuid) and converted_bid_id is not null), 0::bigint, 'failed publishes leave intake pending and unconverted'); -- 30
...
select throws_ok($$select * from public.list_mail_intake_items('53000000-0000-4000-8000-000000000002')$$, '42501', 'An active BUYER membership is required', 'TRADER cannot list prepared intake'); -- 35
reset role;
