...
- Current baseline: trusted organization labels are server-sourced presentation data in the access context and frontend workspace; BUYER bid creation supports advisory browser-local `.msg` draft intake; and a provider-neutral private server-side mail intake foundation stages normalized candidates
