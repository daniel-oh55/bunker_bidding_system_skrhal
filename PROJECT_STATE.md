...
- The BUYER main bid presentation is a full-width responsive operational board: each bid card places its server-returned summary above a per-bid table of BUYER-visible quotes loaded only through `listQuotesForBuyers`. Requested fuel columns follow bid fuel-item order and `total_amount` remains server-authoritative. On effective-open bids, quotes with active access and organization participate in the advisory current-price ranking even though they cannot yet be awarded; inactive quotes remain visible but cannot win. On effective-closed bids, advisory ranking uses server-returned `eligible_for_award`. Awarded cards use authoritative Bid award fields, and cancelled cards show no award candidate. Award remains manual through the existing server-authorized detail flow below the board. Board quote loads use bounded concurrency, isolate non-authorization failures, fail closed on authorization failures, and reject superseded view, membership, and reload results. No authorization scope or backend contract changed.
