...
async function run() {
  ...
  try {
    ...
    const publishSellerOrganizationId = randomUUID(); state.orgs.push(publishSellerOrganizationId); await query("insert into app_private.organizations(id,kind,name,status) values($1,'trader',$2,'active')", [publishSellerOrganizationId, `publish-seller-${publishSellerOrganizationId}`]);
    ...
    const created = await rpc(buyerCaller, 'create_bid', { p_actor_membership_id: buyer.membershipId, p_vessel_voyage: 'REST vessel', p_port_name: 'Busan', p_delivery_window: 'window', p_deadline_at: new Date(Date.now() + 86_400_000).toISOString(), p_responsible_buyer_user_id: null, p_fuel_grades: ['vlsfo'], p_quantities: [10], p_selected_trader_organization_ids: [publishSellerOrganizationId] }, 'buyer publish bid');
    ...
    const secondScope = await rpc(buyerCaller, 'grant_bid_trader_access', { p_actor_membership_id: buyer.membershipId, p_bid_id: state.bidId, p_expected_revision: 2, p_trader_organization_id: traderOther.organizationId }, 'buyer grants a second current scope');
    assert(!secondScope.error && secondScope.data.revision === 3, 'BUYER could not grant a second current TRADER scope.');
    const revoked = await rpc(buyerCaller, 'revoke_bid_trader_access', { p_actor_membership_id: buyer.membershipId, p_bid_id: state.bidId, p_expected_revision: 3, p_trader_organization_id: traderOne.organizationId }, 'buyer revokes one of multiple scopes');
    assert(!revoked.error && revoked.data.revision === 4, 'BUYER could not revoke one of multiple current TRADER scopes.');
    ...
    const regranted = await rpc(buyerCaller, 'grant_bid_trader_access', { p_actor_membership_id: buyer.membershipId, p_bid_id: state.bidId, p_expected_revision: 4, p_trader_organization_id: traderOne.organizationId }, 'buyer regrants scope');
    assert(!regranted.error && regranted.data.revision === 5, 'BUYER could not regrant scope while effective-open.');
    const closed = await rpc(buyerCaller, 'close_bid', { p_actor_membership_id: buyer.membershipId, p_bid_id: state.bidId, p_expected_revision: 5 }, 'buyer closes bid');
    ...
    const awarded = await rpc(buyerCaller, 'award_bid', { p_actor_membership_id: buyer.membershipId, p_bid_id: state.bidId, p_expected_revision: 6, p_quote_id: state.quoteId, p_expected_quote_revision: 2 }, 'buyer awards quote');
