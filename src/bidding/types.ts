...
export type QuoteResponseStatus = 'awaiting' | 'quoted' | 'gave_up';
...
export type TraderBid = Omit<Bid, 'bid_date' | 'created_by' | 'created_by_label' | 'responsible_buyer_user_id' | 'responsible_buyer_label' | 'awarded_quote_id' | 'awarded_trader_organization_id' | 'awarded_trader_organization_label' | 'awarded_total_amount' | 'awarded_at'> & { response_status: QuoteResponseStatus; response_revision: number };
...
const quoteKeys = new Set([
  ...
  'organization_active', 'eligible_for_award', 'is_awarded', 'response_status',
]);
...
export function parseSellerRegistrationRequest(value: unknown): SellerRegistrationRequest | null {
  ...
  return { request_id: r.request_id as string, source: source as SellerRegistrationRequest['source'], requested_organization_name: r.requested_organization_name as string, status: status as SellerRegistrationRequest['status'], revision: revision(r.revision)!, submitted_at: r.submitted_at as string, decided_at: decidedAt, mapped_trader_organization_id: mappedOrganizationId };
}
export function parseSellerRegistrationAdminRequest(value: unknown): SellerRegistrationAdminRequest | null {
  ...
  if (!r || !exactKeys(r, sellerRegistrationAdminRequestKeys) || !id(r.request_id) || !id(r.applicant_user_id) || boundedText(r.applicant_email, 254) === null || !source || !['self_signup', 'invited'].includes(source) || boundedText(r.requested_organization_name, 120) === null || revision(r.revision) === null || !mailIntakeTimestamp(r.submitted_at)) return null;
  return { request_id: r.request_id as string, applicant_user_id: r.applicant_user_id as string, applicant_email: r.applicant_email as string, source: source as SellerRegistrationAdminRequest['source'], requested_organization_name: r.requested_organization_name as string, revision: revision(r.revision)!, submitted_at: r.submitted_at as string };
}
...
export function parseTraderBid(value: unknown): TraderBid | null {
  ...
  const responseStatus = text(r.response_status);
  if (!id(r.id) || !text(r.vessel_voyage) || !text(r.port_name) || !text(r.delivery_window) || nullableDate(r.deadline_at) === undefined || !raw || !statuses.has(raw as BidStatus) || !effective || !statuses.has(effective as BidStatus) || revision(r.revision) === null || !fuelItems(r.fuel_items) || date(r.created_at) === undefined || date(r.updated_at) === undefined || nullableDate(r.closed_at) === undefined || nullableDate(r.cancelled_at) === undefined || !responseStatus || !responseStatuses.has(responseStatus as QuoteResponseStatus) || revision(r.response_revision) === null) return null;
  ...
  return { id: r.id as string, vessel_voyage: r.vessel_voyage as string, port_name: r.port_name as string, delivery_window: r.delivery_window as string, deadline_at: r.deadline_at as string | null, raw_status: raw, effective_status: effective, revision: revision(r.revision)!, fuel_items: fuelItems(r.fuel_items)!, created_at: r.created_at as string, updated_at: r.updated_at as string, closed_at: r.closed_at as string | null, cancelled_at: r.cancelled_at as string | null, response_status: responseStatus as QuoteResponseStatus, response_revision: revision(r.response_revision)! };
}
...
export function parseQuote(value: unknown): Quote | null { const r = record(value); const responseStatus = r && text(r.response_status); if (!r || !exactKeys(r, quoteKeys) || !id(r.id) || !id(r.bid_id) || !id(r.trader_organization_id) || !text(r.trader_organization_label) || revision(r.revision) === null || !id(r.created_by) || !fuelPrices(r.fuel_prices) || finite(r.barge_fee) === null || finite(r.barge_fee)! < 0 || finite(r.total_amount) === null || finite(r.total_amount)! <= 0 || !date(r.created_at) || !date(r.updated_at) || bool(r.access_active) === null || bool(r.organization_active) === null || bool(r.eligible_for_award) === null || bool(r.is_awarded) === null || !responseStatus || !responseStatuses.has(responseStatus as QuoteResponseStatus) || (r.eligible_for_award === true && r.is_awarded === true) || (r.eligible_for_award === true && responseStatus !== 'quoted')) return null; return { id: r.id as string, bid_id: r.bid_id as string, trader_organization_id: r.trader_organization_id as string, trader_organization_label: r.trader_organization_label as string, revision: revision(r.revision)!, created_by: r.created_by as string, fuel_prices: fuelPrices(r.fuel_prices)!, barge_fee: finite(r.barge_fee)!, total_amount: finite(r.total_amount)!, created_at: r.created_at as string, updated_at: r.updated_at as string, access_active: r.access_active as boolean, organization_active: r.organization_active as boolean, eligible_for_award: r.eligible_for_award as boolean, is_awarded: r.is_awarded as boolean, response_status: responseStatus as QuoteResponseStatus }; }
export function parseQuoteResponse(value: unknown): QuoteResponse | null { const r = record(value); const status = r && text(r.response_status); const quoteId = r && nullableId(r.quote_id); const quoteRevision = r && (r.quote_revision === null ? null : revision(r.quote_revision) ?? undefined); return r && id(r.bid_id) && id(r.trader_organization_id) && status && responseStatuses.has(status as QuoteResponseStatus) && revision(r.revision) !== null && quoteId !== undefined && quoteRevision !== undefined && ((quoteId === null) === (quoteRevision === null)) ? { bid_id: r.bid_id as string, trader_organization_id: r.trader_organization_id as string, response_status: status as QuoteResponseStatus, revision: revision(r.revision)!, quote_id: quoteId, quote_revision: quoteRevision } : null; }
export function parseBuyerSellerComparison(value: unknown): BuyerSellerComparison | null {
  const r = record(value);
  const responseStatus = r && text(r.response_status);
  if (!r || !exactKeys(r, buyerSellerComparisonKeys) || !id(r.bid_id) || !id(r.trader_organization_id) || !text(r.trader_organization_label) || bool(r.access_active) === null || bool(r.organization_active) === null || !responseStatus || !responseStatuses.has(responseStatus as QuoteResponseStatus)) return null;
  if (r.quote === null) return responseStatus !== 'quoted' ? { bid_id: r.bid_id as string, trader_organization_id: r.trader_organization_id as string, trader_organization_label: r.trader_organization_label as string, access_active: r.access_active as boolean, organization_active: r.organization_active as boolean, response_status: responseStatus as QuoteResponseStatus, quote: null } : null;
