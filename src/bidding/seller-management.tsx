...
export function SellerManagement({ client, membershipId, reloadVersion = 0, onAuthorizationFailure, onActiveOrganizationsChanged, registrationEnabled = false, accessClient }: {
  ...
  return <section className="panel seller-management" aria-label="SELLER management">
    ...
    {visible ? <div className="seller-management-content">
      ...
      {registrationEnabled && accessClient ? <SellerRegistrationAdministration client={client} accessClient={accessClient} membershipId={membershipId} onAuthorizationFailure={onAuthorizationFailure} onActiveOrganizationsChanged={onActiveOrganizationsChanged} /> : null}
    </div> : null}
  </section>;
}

function SellerRegistrationAdministration({ client, accessClient, membershipId, onAuthorizationFailure, onActiveOrganizationsChanged }: { client: BiddingClient; accessClient: AccessClient; membershipId: string; onAuthorizationFailure: () => void; onActiveOrganizationsChanged: () => unknown }) {
  ...
  const clearAndRecheck = useCallback(() => { setRequests([]); setOrganizations([]); setConfirmation(null); onAuthorizationFailure(); }, [onAuthorizationFailure]);
  const load = useCallback(async () => { setPending(true); const [requestResult, organizationResult] = await Promise.all([client.listSellerRegistrationRequestsForAdmin ? client.listSellerRegistrationRequestsForAdmin(membershipId) : Promise.resolve({ data: null, error: unknownError }), client.listActiveTraderOrganizations(membershipId)]); setPending(false); const error = requestResult.error ?? organizationResult.error; if (error) { setNotice(error.kind === 'authorization' ? 'Registration administration is no longer available.' : error.message); if (error.kind === 'authorization') clearAndRecheck(); return; } setRequests(requestResult.data ?? []); setOrganizations(organizationResult.data ?? []); }, [clearAndRecheck, client, membershipId]);
