...
describe('BiddingClient RPC adapter', () => {
  ...
  it('maps all BUYER methods with the selected membership and only contract arguments', async () => {
    ...
    expect(rpc.mock.calls.map(([, args]) => args)).toEqual([
      ...
      { p_actor_membership_id: id, p_vessel_voyage: 'V', p_port_name: 'P', p_delivery_window: 'W', p_deadline_at: now, p_responsible_buyer_user_id: other, p_fuel_grades: ['vlsfo'], p_quantities: [1], p_selected_trader_organization_ids: [id] },
