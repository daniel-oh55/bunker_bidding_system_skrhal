...
- The Gmail connector is POST-only with `verify_jwt = false` and authenticates `x-gmail-connector-secret` before privileged work; browser JWTs are not authority. It refreshes the configured OAuth grant, verifies the profile exactly, and uses only the readonly Gmail scope. First run saves the current profile history ID and ingests zero messages. Later runs paginate `messageAdded` INBOX history, de-duplicate IDs, request `format=full`, and inspect only inline `text/plain`; HTML, attachments, addresses, raw MIME, and remote resources are excluded. The total decoded text cap is 256 KiB.
- After every retrievable discovered message reaches the existing idempotent ingest RPC, the connector advances the cursor once through CAS. A definitive `users.messages.get` HTTP 404 for a history-discovered message is terminal absence: it is not ingested, its identity is not returned, and only a bounded aggregate is reported; it does not prevent advancement after all other messages succeed. Any other message retrieval/processing or ingest failure leaves the cursor unchanged, ingest SQLSTATE `40001` retries at most three attempts, Gmail history HTTP 404 returns a fixed stale-history error without reset, and a stale cursor revision is never overwritten. After full pagination, the cursor uses the `historyId` returned by the final Gmail history page. `received_at` comes only from Gmail `internalDate`, never deadline authority, and the existing `createBid` contract is unchanged.
