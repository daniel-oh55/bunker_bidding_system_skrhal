...
- After every discovered message reaches the existing idempotent ingest RPC, the connector advances the cursor once through CAS. A message/ingest failure leaves the cursor unchanged, ingest SQLSTATE `40001` retries at most three attempts, Gmail history HTTP 404 returns a fixed stale-history error without reset, and a stale cursor revision is never overwritten. `received_at` comes only from Gmail `internalDate`, never deadline authority, and the existing `createBid` contract is unchanged.
