...
- Supabase access: ten repository migrations and pgTAP tests; the first nine canonical migrations are applied to Production, including `20260821070000_mail_intake_foundation` and `20260821100000_mail_intake_ingress_hardening`; the connector cursor migration is local-only and undeployed
...
- Server intake foundation: the undeployed Gmail Edge Function uses a backend secret key to call only normalized ingress plus two provider-neutral cursor RPCs. It stages bounded candidates in the existing RLS-enabled `app_private` queue; the BUYER frontend still uses only narrow authenticated list/dismiss RPCs and exposes no ingest or connector surface
...
- secret values and backend credentials never enter browser code, Vite variables, repository source values, logs, or test output; server-only runtime names are allowed only under `supabase/functions/**`
...
- ten migrations exist in the repository; only the prior nine reviewed migrations, including canonical mail-intake versions `20260821070000` and `20260821100000`, are applied to Production, and the canonical Vercel Production deployment remains unchanged
...
- manual browser-local `.msg` draft intake, the private normalized server intake boundary, active-BUYER provider-neutral pending list/dismiss UI, and undeployed synthetic-tested Gmail connector code are allowed. Real Gmail/App Password setup, live mailbox connectivity, scheduled polling/webhook/cron, Production connector deployment, manual `.eml`, intake-to-bid conversion, automatic bid creation, historical email migration/import, and operational email fixtures remain outside the architecture; no real operational mail data was introduced
