...
- The repository adds an undeployed Gmail connector foundation for one controlled mailbox. It uses only `https://www.googleapis.com/auth/gmail.readonly`, verifies the configured Gmail profile exactly, establishes a first-run history cutover with zero historical ingestion, and later uses paginated `messageAdded`/`INBOX` history plus full-format message retrieval. It analyzes only inline plain text up to 256 KiB total, reuses the existing advisory parser, calls only the existing ingest RPC, and advances a private provider-neutral cursor through revision CAS after complete success.
...
- manual `.eml` intake; Microsoft Graph integration; real Gmail OAuth configuration, credentials, tokens, mailbox data, or live connection; scheduled polling, webhooks, or cron; Production connector/migration deployment; automatic cursor reset or historical import; intake-to-bid conversion; and automatic bid creation
...
- The Gmail trigger is POST-only and authenticates its dedicated header before any OAuth, Gmail, database, or cursor call. Browser JWTs are not connector authority. Gmail history HTTP 404, any message retrieval/processing failure other than a definitive `users.messages.get` HTTP 404 for a history-discovered message, or stale cursor revision fails closed without automatic reset, historical scan, or cursor advance. A definitive missing message is counted only in a bounded aggregate and is not ingested; after all remaining messages succeed, the cursor may advance. Ingest serialization failures retry at most three attempts.
