...
- The repository adds an undeployed Gmail connector foundation for one controlled mailbox. It uses server-only IMAP with an App Password at fixed TLS `imap.gmail.com:993`, opens `INBOX` read-only, and has no OAuth/Gmail API or OAuth Testing-token dependency. First run establishes `imap-v1:<uidvalidity>:<last_uid>` at the UID high-water mark with zero historical ingestion; later runs use a bounded UID snapshot, analyze only inline plain text up to 256 KiB total, reuse the existing advisory parser, call only the existing ingest RPC, and advance the private provider-neutral cursor through revision CAS after complete success. UIDVALIDITY change fails closed without reset or historical rescan.
...
- manual `.eml` intake; Microsoft Graph integration; real Gmail/App Password configuration, credentials, mailbox data, or live connection; scheduled polling, webhooks, or cron; Production connector/migration deployment; automatic cursor reset or historical import; intake-to-bid conversion; and automatic bid creation
