...
- The Gmail trigger is POST-only and authenticates its dedicated header before any OAuth, Gmail, database, or cursor call. Browser JWTs are not connector authority. Gmail history HTTP 404, any message/ingest failure, or stale cursor revision fails closed without automatic reset, historical scan, or cursor advance; ingest serialization failures retry at most three attempts.
