...
- The frontend creates separate access, bidding, and narrow Realtime invalidation adapters from the same publishable client and never exposes the raw client to components. It selects only server-returned memberships, validates every RPC response before rendering, and clears protected data before an authorization-failure recheck. The sanitized access session contains only email and the authenticated user UUID needed to form that user's own access topic; it contains no token, raw User, metadata, or claims. This is UX/state coordination, not the enforcement layer; bid and quote data still require RLS or server-side functions.
...
- Seven reviewed Supabase migrations, including `20260807010000_current_access_context_organization_label` and `20260808090000_realtime_workspace_notifications`, and controlled BUYER/TRADER provisioning are complete in Production. The reviewed five-field `current_access_context()` contract, its `STABLE` fixed-search-path `SECURITY DEFINER` boundary, and authenticated-only execute privilege were verified after application; no new migration-attributable security advisor finding was found. No real operational bidding data has been migrated or is in use; retained Production records are synthetic smoke records only.
...
- PR #31's mail-intake foundation is merged in the repository, but neither mail-intake migration is applied to Production by this PR. A future connector must tolerate lock waits and treat SQLSTATE `40001` as retryable if it uses stronger isolation or otherwise receives a serialization failure.
