...
- Previously approved Supabase Production baseline migrations and controlled BUYER/TRADER provisioning are complete. The repository organization-label migration is not applied remotely without separate owner approval. No real operational bidding data has been migrated or is in use; the retained Production record is only a synthetic smoke fixture.
