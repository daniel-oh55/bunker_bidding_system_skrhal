...
- The server-side mail intake queue is private, RLS-enabled, and has all direct `public`/`anon`/`authenticated` table privileges revoked. Its ingress RPC is fixed-search-path `SECURITY DEFINER` and grants EXECUTE only to the Supabase service role; it accepts no actor membership or credential/address/raw-message fields. Browser code never receives service-role or provider credentials.
- Intake source identity is exactly the normalized provider, opaque mailbox key, and message ID under a unique database constraint. Ingress uses that constraint as the concurrency boundary, returns the existing internal UUID on duplicates, and never overwrites or reopens the stored row.
